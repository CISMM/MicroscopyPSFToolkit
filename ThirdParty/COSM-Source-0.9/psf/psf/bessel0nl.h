...
/* Bessel function based on Netlib, C verson      Jan 1999 */ 
#ifndef _BESSEL_0_NL_H
#define _BESSEL_0_NL_H
...
/* Type of computer arithmetic from mconf.h */
...
#ifdef DEC
static short R1[] = {0040671,0007734,0001061,0056734};
#define DR1 *(double *)R1
static short R2[] = {0041363,0142445,0030416,0165567};
#define DR2 *(double *)R2
#endif

#ifdef IBMPC
static short R1[] = {0x2bbb,0x8046,0x21fb,0x4017};
#define DR1 *(double *)R1
static short R2[] = {0xdd6f,0xa621,0x78a4,0x403e};
#define DR2 *(double *)R2
#endif

#ifdef MIEEE
static short R1[] = {0x4017,0x21fb,0x8046,0x2bbb};
#define DR1 *(double *)R1
static short R2[] = {0x403e,0x78a4,0xa621,0xdd6f};
#define DR2 *(double *)R2
#endif
...
class J0nl : public Functor<T> {
  ...
  public:
	...
	if ( x <= 5.0 )
	{
		...
		return( 1.0 - xsq/4.0 );
	    p = (xsq - DR1) * (xsq - DR2);
	    p = p * polevl( xsq, (T*)RP, 3)/p1evl( xsq, (T*)RQ, 8 );
	    return ( p );
	}
	T w = 5.0/x;
	T xn = x - M_PI_4;
	T q = 25.0/(x*x);
	p = polevl( q, (T*)PP, 6)/polevl( q, (T*)PQ, 6 );
	q = polevl( q, (T*)QP, 7)/p1evl( q, (T*)QQ, 7 );
  ...
  protected:
    ...
    // not allowed
    J0nl( J0nl<T>& );
    J0nl& operator=( J0nl<T>& );

};

}

#endif // _BESSEL_0_NL_H
