...
main (int argc, char* argv[])
{
    ...
    ValueArg<std::string> refArg("u","reference", "Reference Image", false, "ref. wu", "reference");
    ...
    SwitchArg ellipsoidArg("d","ellipsoid", "Ellipsoid", false);
    ...
    SwitchArg variantArg("V", "variant", "Create Space Variant Image", false);
    ValueArg<int> numberArg("f","number", "Number of Strata", false, 1, "number");
    ValueArg<int> startArg("g","number", "Start of Strata", false, 1, "start");
    ValueArg<int> sizeArg("h","number", "Size of Strata", false, 1, "size");
...
*/
    cmdLine.add(ellipsoidArg);
    cmdLine.add(boxArg);
    cmdLine.add(pointArg);
...
*/
    ...
    cmdLine.add(infoArg);
    cmdLine.add(addArg);
    cmdLine.add(removeArg);
    cmdLine.add(transformArg);
    cmdLine.add(resampleArg);
    cmdLine.add(convertArg);
    cmdLine.add(convolveArg);
    cmdLine.add(objectArg);
    ...
    if ( cliArg.getValue() )  
    {
        ...
        if ( removeArg.getValue() )
        {
            ToolMgr toolMgr( dataArg.getValue(), outputArg.getValue() );
            	...
            	//Not yet sopport multiple PSFs for CLI
            if ( toolMgr.removeHeader(1) )
            {
                return 0;
            }
        }
        ...
        if ( compareArg.getValue() )
        {
         ...
         if ( variantArg.getValue() )
         {
             ToolMgr toolMgr( dataArg.getValue(), refArg.getValue(), outputArg.getValue() );
             if ( toolMgr.variant(numberArg.getValue(), startArg.getValue(), sizeArg.getValue() ))
             {
                 return 0;
             }
         }
